...
@Query("SELECT new com.example.esprit.dto.TaskSummaryDTO(t.id, t.title, t.description, t.dueDate, t.status, t.project.id) " +
       "FROM Task t JOIN t.assignments a WHERE a.group.id = :groupId AND t.isVisible = true")
List<TaskSummaryDTO> findVisibleTasksByGroupId(@Param("groupId") Long groupId);
